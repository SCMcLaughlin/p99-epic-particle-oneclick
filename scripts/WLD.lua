...
function WLD.new(data, len)
    ...
    local stringsCopy = BinUtil.Char.Array(header.stringBlockLen)
    ffi.copy(stringsCopy, stringBlock, header.stringBlockLen)

    local wld = {
        _stringBlock        = stringsCopy,
...
function WLD:getHeader()
    return self._header, Header:sizeof()
end
...
function WLD:decodeTextureNames()
    for i, f03 in self:getFragsByType(0x03) do
...
function WLD:getFragsByIndex()
    return ipairs(self._fragsByIndex)
end

function WLD:getFragsByType(t)
    return ipairs(self._fragsByType[t] or {})
end
